...
const UrlResolver   = require('./url_resolver')
  ...
  onMessage(message) {
    ...
    co(function* () {
      try {
        ...
        if (_this.startedAt > ts || isBot) { return }
        ...
        results.forEach(r => co(function* () {
          try {
            const result  = yield r
            const botUser = result.botUser || _this.botUser

            yield postMessage(channel, result.message, botUser)
